...
var ACCESSORIES = {
  none: { label: '无' },
  'iron-basin': { label: '铁盆（头顶饭盆）', src: '/dsh-whale/iron-basin.png' }
}
var accessory = 'none'
var accessoryChanged = false
...
var css = [
  ...
  '.dshwv-img-basin{clip-path:inset(15% 0 0 0)}',
  '.dshwv-accessory{position:absolute;left:46%;top:44.5%;width:65%;height:26%;transform:rotate(8deg);transform-origin:50% 85%;pointer-events:none;-webkit-user-drag:none;user-select:none}',
...
accessoryImg.className = 'dshwv-accessory'
accessoryImg.alt = ''
accessoryImg.draggable = false
accessoryImg.hidden = true
var accessoryHitCanvas = document.createElement('canvas')
accessoryHitCanvas.width = 256
accessoryHitCanvas.height = 128
var accessoryHitReady = false
accessoryImg.addEventListener('load', function () {
  var ctx = accessoryHitCanvas.getContext('2d')
  ctx.clearRect(0, 0, 256, 128)
  ctx.drawImage(accessoryImg, 0, 0, 256, 128)
  accessoryHitReady = true
})
accessoryImg.addEventListener('error', function () {
  accessoryHitReady = false
  accessoryImg.hidden = true
  img.classList.remove('dshwv-img-basin')
})
...
var accessorySelect = document.createElement('select')
accessorySelect.className = 'dshwv-sound'
accessorySelect.setAttribute('aria-label', '挂饰')
Object.keys(ACCESSORIES).forEach(function (key) {
  accessorySelect.appendChild(soundOpt(key, ACCESSORIES[key].label))
})
accessorySelect.addEventListener('change', function () {
  accessoryChanged = true
  applyAccessory(accessorySelect.value)
  saveConfig()
})
function applyAccessory(value) {
  accessory = Object.prototype.hasOwnProperty.call(ACCESSORIES, value) ? value : 'none'
  accessorySelect.value = accessory
  var item = ACCESSORIES[accessory]
  accessoryHitReady = false
  accessoryImg.hidden = !item.src
  img.classList.toggle('dshwv-img-basin', !!item.src)
  if (item.src) accessoryImg.src = item.src
  else accessoryImg.removeAttribute('src')
}
...
var accessoryRow = menuRow()
accessoryRow.appendChild(menuLabel('挂饰'))
accessoryRow.appendChild(accessorySelect)
menuBox.appendChild(accessoryRow)
...
var RANDOM_GROUPS = [
  // 权重已均分（6 组各 1）：点击后六种内容等概率出现（各约 1/6）
  { w: 1, lines: buildGroup1 },
  { w: 1, lines: function () { return singleCenter('B', pickOne(['好模型... ↓', '好女孩...↓'])) } },
  { w: 1, lines: function () { return singleCenter('A', pickOne(['不知道用户有什么用，先赶走吧~', '我...我...我也要挣钱吗？', '我去吃饭啦，测完叫我', '压力一只蓝色大肥鱼？！', 'DeepSleep...', '坏了...用户彻底怒了！']), '', true) } },
  { w: 1, lines: function () { return { gif: true } } },
  { w: 1, lines: function () { return singleCenter('A', pickOne(['你目录里的dsh是什么...大烧货吗...?', '恭喜你实现token自由！token全跑了！', '真当我是便宜货啊...']), '', true) } },
...
function saveConfig() {
  try {
    fetch(SIZE_URL, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scale: state.scale, sound: soundOn, vol: soundVol, soundSet: soundSet, accessory: accessory, usageMode: usageMode, peakMode: peakMode, bubbleOn: bubbleOn, turnCostOn: turnCostOn, turnCostCloseMs: turnCostCloseMs, scrollGapOn: scrollGapOn, scrollGapPx: scrollGapPx }) })
...
function isWhaleHit(e) {
  // 挂饰和本体分别采样透明度；盆沿可拖拽，透明边角仍允许点击下层页面。
  if (!accessoryImg.hidden && accessoryHitReady) {
    var br = body.getBoundingClientRect()
    var bx = (e.clientX - br.left) / br.width
    if (state.h === 'left') bx = 1 - bx
    var computed = getComputedStyle(accessoryImg)
    var origin = computed.transformOrigin.split(' ').map(Number.parseFloat)
    var point = new DOMMatrix(computed.transform).inverse().transformPoint({
      x: bx * body.offsetWidth - accessoryImg.offsetLeft - origin[0],
      y: (e.clientY - br.top) / br.height * body.offsetHeight - accessoryImg.offsetTop - origin[1]
    })
    var ax = (point.x + origin[0]) / accessoryImg.offsetWidth
    var ay = (point.y + origin[1]) / accessoryImg.offsetHeight
    if (ax >= 0 && ay >= 0 && ax < 1 && ay < 1) {
      if (accessoryHitCanvas.getContext('2d').getImageData(Math.min(255, Math.floor(ax * 256)), Math.floor(ay * 128), 1, 1).data[3] > 10) return true
    }
  }
  ...
  try {
    ...
    if (img.classList.contains('dshwv-img-basin') && ly < 610 * 0.15) return false
...
fetch(SIZE_URL, { cache: 'no-store' })
  .then(function (r) { return r.json() })
  .then(function (d) {
    if (!accessoryChanged) applyAccessory(d && d.accessory)
...
function apply(ctx) {
        ...
        try {
          ...
          if (parsed && typeof parsed.scale === 'number') {
            return {
              ...
              accessory: parsed.accessory === 'iron-basin' ? 'iron-basin' : 'none',
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, accessory) {
      ...
      const body = JSON.stringify({
        ...
        accessory: accessory === 'iron-basin' ? 'iron-basin' : 'none',
        usageMode: um,
        peakMode: pm,
        bubbleOn: bo,
        turnCostOn: tco,
    ...
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/iron-basin.png',
      handler: (req, res) => {
        let bytes
        try {
          bytes = fs.readFileSync(path.join(PACKAGE_ROOT, 'assets', 'iron-basin.png'))
        } catch (err) {
          if (err.code !== 'ENOENT') throw err
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('iron basin image unavailable')
          return
        }
        res.writeHead(200, {
          'Content-Type': 'image/png',
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            if (parsed.accessory !== undefined && parsed.accessory !== 'none' && parsed.accessory !== 'iron-basin') {
              res.writeHead(400, JSON_HEADERS)
              res.end(JSON.stringify({ ok: false, error: 'invalid accessory' }))
              return
            }
            const accessory = parsed.accessory === undefined ? (readSizeConfig() || {}).accessory : parsed.accessory
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, accessory)
